...
const storage_adapter_1 = require("./infrastructure/storage-adapter");
const compiler_engine_1 = require("./core/compiler-engine");
const distiller_1 = require("./ingestion/distiller");
const resolve_document_content_1 = require("./ingestion/resolve-document-content");
const contracts_1 = require("./schemas/contracts"); // Import the new schema
const zod_1 = require("zod"); // For schema validation
const bundle_store_1 = require("./infrastructure/bundle-store");
...
const validator_1 = require("./partners/validator");
const bundle_1 = require("./partners/bundle");
const partner_ingest_1 = require("./partners/partner-ingest");
const source_upload_1 = require("./partners/source-upload");
const partner_draft_update_1 = require("./partners/partner-draft-update");
const partner_drafts_list_1 = require("./partners/partner-drafts-list");
const publisher_1 = require("./partners/publisher");
const partner_assist_1 = require("./partners/partner-assist");
const license_sync_1 = require("./partners/license-sync");
const partner_eval_status_1 = require("./partners/partner-eval-status");
...
const indexerPool = new pg_1.Pool({
    connectionString: process.env.COLD_TIER_URL || process.env.DATABASE_URL || 'postgres://postgres:postgres@localhost:5436/postgres',
});
const indexerEmbeddings = process.env.NODE_ENV === 'test' ? new embeddings_mock_1.MockEmbeddingsClient() : new embeddings_1.GeminiEmbeddingsClient();
...
exports.app.use('/v1/*', async (c, next) => {
...
exports.app.post('/v1/ingest', async (c) => {
    try {
        const rawBody = await c.req.json();
        const parsedRequest = contracts_1.IngestRequestV1Schema.parse(rawBody);
        ...
        for (const doc of documents) {
            ...
            try {
                ...
                const markdownContent = await (0, resolve_document_content_1.resolveDocumentContent)(doc, distiller);
...
exports.app.get('/v1/jobs/:id', async (c) => {
    try {
...
exports.app.post('/pubsub', async (c) => {
...
exports.app.post('/internal/index-delta', async (c) => {
    ...
    try {
        const rawBody = await c.req.json();
        const { tenantId } = zod_1.z.object({ tenantId: zod_1.z.string().min(1) }).parse(rawBody);
        const store = new bundle_store_1.BundleStore({ tenantId });
        const result = await (0, indexer_1.indexDelta)(tenantId, {
            pool: indexerPool,
            store,
...
exports.app.post('/internal/partner-validate', async (c) => {
    const M2M_API_KEY = process.env.M2M_API_KEY;
    if (!M2M_API_KEY) {
    ...
    try {
        ...
        const body = zod_1.z.object({
            ...
            package_slug: zod_1.z.string().optional(),
            for_publish: zod_1.z.boolean().optional(),
            package_paths: zod_1.z.array(zod_1.z.string()).optional(),
        }).parse(rawBody);
        ...
        if (!packagePaths && body.package_slug) {
            ...
            const packageIdCandidate = zod_1.z.string().uuid().safeParse(body.package_slug);
            if (!packageIdCandidate.success) {
                return c.json({
                    error: 'Not Implemented',
                    details: 'package_slug no resuelve a package_id: okf_partner_concepts (migrations/007) no tiene columna package_slug y no hay mapeo slug→id accesible desde este servicio. Pasa package_paths explícitamente, o un package_slug que sea el UUID de package_id.',
                }, 501);
            }
            const { rows } = await indexerPool.query('SELECT DISTINCT path FROM okf_partner_concepts WHERE partner_id = $1 AND package_id = $2', [body.partner_id, packageIdCandidate.data]);
            packagePaths = rows.map((r) => r.path);
        }
        const report = (0, validator_1.validateConcept)(body.markdown, {
            level: 'n3',
            packagePaths,
...
// Body: {partner_id}
exports.app.post('/internal/partner-bundle-create', async (c) => {
    const M2M_API_KEY = process.env.M2M_API_KEY;
    if (!M2M_API_KEY) {
    ...
    try {
        const rawBody = await c.req.json();
        const { partner_id } = zod_1.z.object({ partner_id: zod_1.z.string().uuid() }).parse(rawBody);
        const result = await (0, bundle_1.createPartnerBundleWithStorage)(partner_id);
        return c.json({
            partner_id,
...
exports.app.post('/internal/partner-ingest', async (c) => {
    const M2M_API_KEY = process.env.M2M_API_KEY;
    if (!M2M_API_KEY) {
    ...
    try {
        ...
        const input = zod_1.z.object({
            partner_id: zod_1.z.string().uuid(),
            package_slug: zod_1.z.string().min(1),
            document: zod_1.z.object({
                filename: zod_1.z.string().min(1),
                content_base64: zod_1.z.string().optional(),
                text: zod_1.z.string().optional(),
            }).optional(),
            source_gcs_object: zod_1.z.string().min(1).optional(),
        }).refine((v) => v.document || v.source_gcs_object, {
            message: 'Debe traer document o source_gcs_object',
        }).parse(rawBody);
        const store = new bundle_store_1.BundleStore({
            tenantId: input.partner_id,
        });
        const result = await (0, partner_ingest_1.ingestPartnerDocument)(input, store);
        return c.json(result, 200);
    }
    catch (error) {
        if (error instanceof zod_1.z.ZodError) {
...
exports.app.post('/internal/partner-source-upload', async (c) => {
    const M2M_API_KEY = process.env.M2M_API_KEY;
    if (!M2M_API_KEY) {
    ...
    try {
        ...
        const input = zod_1.z.object({
            partner_id: zod_1.z.string().uuid(),
            filename: zod_1.z.string().min(1),
            content_base64: zod_1.z.string().min(1),
        }).parse(rawBody);
        const store = new bundle_store_1.BundleStore({
            tenantId: input.partner_id,
        });
        const result = await (0, source_upload_1.uploadPartnerSource)({ partner_id: input.partner_id, filename: input.filename, content_base64: input.content_base64 }, store);
        ...
        if (error instanceof source_upload_1.PartnerSourceTooLargeError) {
...
exports.app.post('/internal/partner-draft-update', async (c) => {
    const M2M_API_KEY = process.env.M2M_API_KEY;
    if (!M2M_API_KEY) {
    ...
    try {
        ...
        const input = zod_1.z.object({
            partner_id: zod_1.z.string().uuid(),
            draft_path: zod_1.z.string().min(1),
            markdown: zod_1.z.string().min(1),
        }).parse(rawBody);
        const store = new bundle_store_1.BundleStore({
            tenantId: input.partner_id,
        });
        const result = await (0, partner_draft_update_1.updatePartnerDraft)(input, store);
        return c.json(result, 200);
    }
    catch (error) {
...
exports.app.post('/internal/partner-drafts-list', async (c) => {
    const M2M_API_KEY = process.env.M2M_API_KEY;
    if (!M2M_API_KEY) {
    ...
    try {
        ...
        const input = zod_1.z.object({
            partner_id: zod_1.z.string().uuid(),
            package_slug: zod_1.z.string().min(1).optional(),
        }).parse(rawBody);
        const store = new bundle_store_1.BundleStore({ tenantId: input.partner_id });
        const result = await (0, partner_drafts_list_1.listPartnerDrafts)(input, store, indexerPool);
        return c.json(result, 200);
    }
    catch (error) {
        if (error instanceof zod_1.z.ZodError) {
...
exports.app.post('/internal/partner-draft-get', async (c) => {
    const M2M_API_KEY = process.env.M2M_API_KEY;
    if (!M2M_API_KEY) {
    ...
    try {
        ...
        const store = new bundle_store_1.BundleStore({ tenantId: input.partner_id });
        const result = await (0, partner_drafts_list_1.getPartnerDraft)(input, store);
        return c.json(result, 200);
    }
    catch (error) {
        if (error?.code === 422) {
...
const PartnerAssistFindingSchema = zod_1.z.object({
    rule_id: zod_1.z.string(),
    level: zod_1.z.enum(['n1', 'n2', 'n3']).optional(),
    severity: zod_1.z.enum(['error', 'warn']).optional(),
    message_es: zod_1.z.string(),
    line: zod_1.z.number().optional(),
    fix: zod_1.z.object({
        kind: zod_1.z.enum(['set_field', 'replace_text']),
        description_es: zod_1.z.string(),
        value: zod_1.z.string().optional(),
        from: zod_1.z.string().optional(),
        to: zod_1.z.string().optional(),
    }).optional(),
});
exports.app.post('/internal/partner-assist', async (c) => {
    const M2M_API_KEY = process.env.M2M_API_KEY;
    if (!M2M_API_KEY) {
    ...
    try {
        const rawBody = await c.req.json();
        const input = zod_1.z.object({
            mode: zod_1.z.enum(['draft_from_source', 'reorganize', 'fix_findings']),
            partner_id: zod_1.z.string().uuid(),
            markdown: zod_1.z.string().min(1).optional(),
            source_gcs_objects: zod_1.z.array(zod_1.z.string().min(1)).optional(),
            findings: zod_1.z.array(PartnerAssistFindingSchema).optional(),
            concept_type: zod_1.z.string().optional(),
            system: zod_1.z.string().optional(),
        }).parse(rawBody);
        const store = new bundle_store_1.BundleStore({ tenantId: input.partner_id });
        const result = await (0, partner_assist_1.runPartnerAssist)(input, store);
        return c.json(result, 200);
    }
    catch (error) {
        if (error instanceof partner_assist_1.PartnerAssistInputError) {
            return c.json({ error: error.message }, 422);
        }
        if (error instanceof partner_assist_1.PartnerAssistLlmError) {
...
exports.app.post('/internal/partner-publish', async (c) => {
    const M2M_API_KEY = process.env.M2M_API_KEY;
    if (!M2M_API_KEY) {
    ...
    try {
        ...
        const input = zod_1.z.object({
            partner_id: zod_1.z.string().uuid(),
            partner_slug: zod_1.z.string().regex(/^[a-z0-9][a-z0-9-]{2,39}$/),
            package_id: zod_1.z.string().uuid(),
            package_slug: zod_1.z.string().regex(/^[a-z0-9][a-z0-9-]{2,59}$/),
            version: zod_1.z.number().int().min(1),
            draft_paths: zod_1.z.array(zod_1.z.string().min(1)).min(1),
        }).parse(rawBody);
        const result = await (0, publisher_1.publishPartnerPackage)(input, {
            pool: indexerPool,
            embeddings: indexerEmbeddings,
        });
        return c.json(result, 200);
        ...
        if (error instanceof publisher_1.PartnerPublishChainBrokenError) {
            return c.json({ error: error.message }, 409);
        }
        if (error instanceof publisher_1.PartnerDraftsNotFoundError) {
            return c.json({ error: error.message, missing: error.missing }, 404);
        }
        if (error instanceof publisher_1.PartnerPublishValidationError) {
            return c.json({ error: error.message, reports: error.reports }, 422);
        }
        if (error instanceof publisher_1.PartnerPublishPartialFailureError) {
...
exports.app.post('/internal/partner-license-sync', async (c) => {
    const M2M_API_KEY = process.env.M2M_API_KEY;
    if (!M2M_API_KEY) {
    ...
    try {
        const rawBody = await c.req.json();
        const input = license_sync_1.PartnerLicenseSyncInputSchema.parse(rawBody);
        if (input.action === 'upsert') {
            const license = input.license;
            await (0, license_sync_1.upsertLicense)(indexerPool, {
            ...
            await (0, license_sync_1.deleteLicense)(indexerPool, input.contract_id);
        }
...
exports.app.get('/internal/partner-package-eval-status', async (c) => {
    const M2M_API_KEY = process.env.M2M_API_KEY;
    if (!M2M_API_KEY) {
    ...
    try {
        ...
        const status = await (0, partner_eval_status_1.getPartnerPackageEvalStatus)(indexerPool, query.package_id, query.partner_id);
...
exports.app.post('/ingest/telegram', async (c) => {
    const authHeader = c.req.header('Authorization');
    const BEARER_TOKEN = process.env.TESEO_API_KEY; // Assuming an API key env var
    if (!BEARER_TOKEN) {
        console.error('TESEO_API_KEY is not set. M2M authentication cannot be performed.');
    ...
    try {
        ...
        const tenantId = body['tenant_id']; // E11-H3: Expect tenant_id from form data
